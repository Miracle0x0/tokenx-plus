  ...
  cat > bun.lock <<EOF_LOCK
{
  ...
  "workspaces": {
    "packages/tokenx": {
      ...
      "version": "${bun_version}",
      "optionalDependencies": {
        "@juya-ai/tokenx-darwin-arm64": "${bun_version}",
        "@juya-ai/tokenx-linux-x64-gnu": "${bun_version}",
        "@juya-ai/tokenx-win32-x64-msvc": "${bun_version}",
      },
    },
    "packages/tokenx-darwin-arm64": {
...
create_fixture() {
  ...
  cp "${RELEASE_MANIFESTS_SCRIPT}" "${work}/scripts/release-manifests.py"
}
...
test_rejects_stale_workspace_versions_in_bun_lock() {
  local work="${TMP_DIR}/stale-bun-lock"
  create_fixture "${work}"
  (
    cd "${work}"
    write_release_manifests "3.0.0" "3.0.0" "2.1.3"

    local output="${TMP_DIR}/stale-bun-lock-output.txt"
    if bash scripts/check-version-coherence.sh --expect-version "3.0.0" >"${output}" 2>&1; then
      echo "Expected stale bun.lock workspace versions to fail" >&2
      return 1
    fi

    grep -q "bun.lock: packages/tokenx must contain direct version 3.0.0" "${output}"
  )
}

test_accepts_matching_workspace_versions() {
  local work="${TMP_DIR}/matching-locks"
  create_fixture "${work}"
  (
    cd "${work}"
    write_release_manifests "3.0.0"
    bash scripts/check-version-coherence.sh --expect-version "3.0.0" >"${TMP_DIR}/matching-locks-output.txt" 2>&1
  )
}
...
test_rejects_unsupported_platform_package() {
...
PY
    ...
    grep -q "Expected platform packages" "${output}"
    grep -q "@juya-ai/tokenx-linux-riscv64-gnu" "${output}"
  )
}

test_rejects_missing_canonical_platform() {
...
PY
    ...
    grep -q "Expected platform packages" "${output}"
    grep -q "@juya-ai/tokenx-win32-x64-msvc" "${output}"
  )
}
