...
PACKAGE_FILES_CHECK="${ROOT_DIR}/scripts/check-npm-package-files.sh"
PACKAGE_FILES_STAGE="${ROOT_DIR}/scripts/stage-npm-package-files.sh"
TMP_DIR="$(mktemp -d)"
...
write_fake_npm() {
  local path="$1"
  if [[ -d packages/tokenx ]]; then
    printf 'fixture license\n' > LICENSE
    printf 'fixture notice\n' > NOTICE
    cp LICENSE NOTICE packages/tokenx/
    printf '# Fixture package\n' > packages/tokenx/README.md
  fi
  cat > "${path}" <<'EOF_NPM'
#!/usr/bin/env bash
set -euo pipefail
...
test_publish_skips_existing_target_version_during_recovery() {
  ...
  cp "${PACKAGE_FILES_CHECK}" "${work}/scripts/check-npm-package-files.sh"
  cp "${PACKAGE_FILES_STAGE}" "${work}/scripts/stage-npm-package-files.sh"
  (
    cd "${work}"
    cat > packages/tokenx/package.json <<'EOF_MANIFEST'
{
