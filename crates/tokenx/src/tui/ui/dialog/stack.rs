...
impl DialogStack {
    ...
    pub fn render(&self, frame: &mut Frame, viewport: Rect) -> Option<Rect> {
    ...
    pub fn handle_key(&mut self, key: KeyEvent) -> DialogResult {
        if self.stack.is_empty() {
            return DialogResult::Ignored("no dialog");
        }
        ...
        DialogResult::Ignored("no dialog")
    }

    /// Handle a mouse event and report the dialog-level outcome.
    pub fn handle_mouse(&mut self, event: MouseEvent, top_rect: Option<Rect>) -> DialogResult {
        if self.stack.is_empty() {
            return DialogResult::Ignored("no dialog");
        }
        ...
        if let (Some(top), Some(rect)) = (self.stack.last_mut(), top_rect) {
            let result = top.handle_mouse(event, rect);
            return self.apply_result(result);
        }

        DialogResult::Ignored("dialog area unavailable")
    }

    fn apply_result(&mut self, result: DialogResult) -> DialogResult {
        match result {
            DialogResult::Handled => DialogResult::Handled,
            DialogResult::Ignored(reason) => DialogResult::Ignored(reason),
            DialogResult::Close => {
                self.close();
                DialogResult::Close
            }
            DialogResult::Submit(command) => {
                self.close();
                DialogResult::Submit(command)
            }
        }
